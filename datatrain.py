...
class TrainData(data.Dataset):
    def __init__(self, exocentric_root, egocentric_root, resize_size=256, crop_size=224, divide="Unseen", num_exo=3):
        ...
        self.transform = transforms.Compose([
            ...
            transforms.Resize(size=(crop_size, crop_size),interpolation=transforms.InterpolationMode.BILINEAR,
            antialias=True),
        ])
    ...
    def __getitem__(self, item):
        ...
        if len(exocentrics) <= self.num_exo:  # If it is less than 3, select all of them and fill them up
            start = 0
            ...
            for i in range(len(exocentrics), self.num_exo):
                exocentric_images.append(tmp_exo)
        else:
            start = random.randint(0, len(exocentrics) - self.num_exo - 1)  # If more than 3, choose 3 randomly
            for idx in range(start, start + self.num_exo):
