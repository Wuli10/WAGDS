...
from models.WAGDS import MODEL
...
args.save_path = os.path.join(args.save_root, args.model_name, time_str)  # save model parameters
...
shutil.copy(f'./models/{args.model_name}.py', args.save_path)

shutil.copy(f'./train.py', args.save_path)
...
if __name__ == '__main__':
    if args.phase == 'train':
        ...
        trainset = datatrain.TrainData(exocentric_root=args.exocentric_root,
                             ...
                             resize_size=args.resize_size, crop_size=args.crop_size, divide=args.divide, num_exo=args.num_exo)
        ...
        model, par = MODEL(args, num_classes=args.num_classes,
                           pretrained=True, n=args.num_exo, D=args.D, n_layer=args.n_layer)    # par: name of the model's parameters in clip's pretrained model 
        model= model.cuda()
        model.train()
        ...
        best_kld_cam = [1000 for _ in range(args.num_exo)]  
        best_kld_cam1 = [1000 for _ in range(args.num_exo)]
        best_kld_finalcam = [1000 for _ in range(args.num_exo)]
        print('Train')
        #args.epochs
        for epoch in range(0, args.epochs):
            ...
            for step, (exocentric_image, egocentric_image, label, name) in enumerate(MyDataLoader):
                ...
                exo_out_list, ego_out_list, text_list, image_list, text_exo_list, image_exo_list, sim_loss_list, cam1_list, cam_list = model(exocentric_images, egocentric_image, label, name)   # name means affordance type  
                ...
                final_cam_list = []
                for i in range(len(exo_out_list)):
                    loss_cls_ego += loss_func(ego_out_list[i], label)
                    loss_cls_exo += loss_func(exo_out_list[i], label)
                    clip_loss_ego += loss_func(image_list[i], label)
                    clip_loss_exo += loss_func(image_exo_list[i], label)
                    sim_loss += sim_loss_list[i].mean() * args.batch_size
                    
                    cam1_list[i][cam_list[i] <args.threshold]=0
                    final_cam_list.append(batch_minmax_normalize(cam_list[i] + cam1_list[i]))



                loss_cls_ego /= len(exo_out_list)
                loss_cls_exo /= len(exo_out_list)
                clip_loss_ego /= len(exo_out_list)
                clip_loss_exo /= len(exo_out_list)
                sim_loss /= len(exo_out_list)
                


                
                loss_cls = loss_cls_ego + loss_cls_exo
                clip_loss = clip_loss_ego + clip_loss_exo
                loss = 0.5 * loss_cls.cuda() + sim_loss.cuda() + clip_loss.cuda()
                ...
                b, h= exo_out_list[0].shape
                exo_out = exo_out_list[0]
                ego_out = ego_out_list[0]
                ...
                if (step + 1) % args.show_step == 0 or step==0:
                    print(
                        '{} \t Epoch:[{}/{}]\tstep:[{}/{}] \t cls_loss_ego: {:.3f}\t cls_loss_exo: {:.3f}\t sim_loss: {:.3f}\t  clip_loss_ego: {:.3f}\t clip_loss_exo: {:.3f}\t exo_acc: {:.2f}% \t ego_acc: {:.2f}%'.format(
                            args.phase, epoch + 1, args.epochs, step + 1, len(MyDataLoader), loss_cls_ego.item(), loss_cls_exo.item(), sim_loss.item(), clip_loss_ego.item(), clip_loss_exo.item(),
            ...
            masks = torch.load(GT_path) # GT_path
            for step, (egocentric_image, label, mask_path, name) in enumerate(MytestDataLoader):
                ...
                for i in range(len(cam_list)):
                    ...
                    if i not in KLs_finalcam_dict:
                        KLs_finalcam_dict[i] = []
                    if i not in KLs_cam_dict:
                        KLs_cam_dict[i] = []
                    if i not in KLs_cam1_dict:
                        KLs_cam1_dict[i] = []
            ...
            for i in range(len(KLs_finalcam_dict)):
                ...
                # print("epoch=" + str(epoch) + " " + "mKLD_cam_" + str(i) + "=" + str(mKLD), flush=True)
                # if mKLD < best_kld_cam[i]:
                #     best_kld_cam[i]= mKLD
                #     torch.save(model.state_dict(), os.path.join(args.save_path, f'best_model_cam_{i}.pth'))

                # cam1_dict = KLs_cam1_dict[i]
                # mKLD = sum(cam1_dict) / len(cam1_dict)
                # print("epoch=" + str(epoch) + " " + "mKLD_cam1_" + str(i) + " = " + str(mKLD), flush=True)
                # if mKLD < best_kld_cam1[i]:
                #     best_kld_cam1[i] = mKLD
                #     torch.save(model.state_dict(), os.path.join(args.save_path, f'best_model_cam1_{i}.pth'))
                finalcam_dict = KLs_finalcam_dict[i]
                mKLD = sum(finalcam_dict) / len(finalcam_dict)
                print("epoch=" + str(epoch) + " " + "mKLD_finalcam_" + str(i) + " = " + str(mKLD), flush=True)
                if  mKLD < best_kld_finalcam[i]:
                    best_kld_finalcam[i] = mKLD
                    torch.save(model.state_dict(), os.path.join(args.save_path, f'best_model_finalcam_{i}.pth'))
