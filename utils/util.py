...
class PairedTransform:
    ...
    def train_transform(self, image, heatmap):
        ...
        assert (heatmap.shape[1], heatmap.shape[0]) == image.size, 'image and heatmap sizes mismatch (%s vs. %s)' % (
        image.size, heatmap.shape)

        # resize
        image = TF.resize(image, size=(256, 256))
        heatmap = \
        F.interpolate(heatmap.unsqueeze(0).unsqueeze(0), size=image.size, mode='bilinear', align_corners=False)[0][0]
